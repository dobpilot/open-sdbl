...
разыменование ссылок любой глубины (включая составные на первом шаге), представления значений, отбор по
...
регистров сведений и накопления (срез первых и последних, остатки, обороты) и
построчные ограничения доступа `РАЗРЕШЕННЫЕ`.
...
расчёта,
`ОстаткиИОбороты` и периодичность у `Обороты`, критерии отбора, журналы
документов и внешние источники данных. `Остатки` доступны только у регистра с
остатками: у оборотного регистра это диагностика. Разыменование выполняется на один шаг, источники соединяются только
явным `СОЕДИНЕНИЕ` (перечисление через запятую не поддерживается), а
ограничения доступа не читаются из ролей: условия задаёт приложение через
callback. Проект развивается и пока не является полной заменой языка запросов
платформы 1С.
...
в `ON`, поэтому семантика внешних соединений не меняется. Не допускаются
`.Реквизит` после `ВЫРАЗИТЬ(… КАК Справочник.X)` и разыменование в `ПО` у
`ПОЛНОЕ СОЕДИНЕНИЕ`.
...
`ПОЛНОЕ СОЕДИНЕНИЕ` допустимо только единственным в ветви, требует в `ПО`
прямых полей и не допускает агрегатов над собой: оно транспонируется в
`UNION ALL`. Один и тот же
объект можно присоединять несколько раз под разными псевдонимами, а
разыменование работает через любой источник цепочки (один шаг):
