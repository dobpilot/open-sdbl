...
`ХранилищеЗначения` (ссылка `STORHDR` → части в `binarydata` → скобочная
сериализация), оттуда приходят `ВерсииШаблоновОграниченияДоступа` и четыре
...
Значение, заданное руками через `\session`, не перетирается. На демо-базе
...
`\refresh`. Права вне `Чтения` в запросах не участвуют, но в модели
...
Роли расширений (`ConfigCas`) разбираются теми же функциями, но связать
запись прав с ролью расширения без индекса контейнера пока нельзя.
