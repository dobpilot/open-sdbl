...
разыменование ссылок (включая составные), представления значений, виртуальные
таблицы регистров сведений и накопления (срез первых и последних, остатки,
обороты) и построчные ограничения доступа `РАЗРЕШЕННЫЕ`.

Не поддержаны, в частности: `ИТОГИ`, `АВТОУПОРЯДОЧИВАНИЕ`, `ИЕРАРХИЯ` и
`В ИЕРАРХИИ`, `МЕЖДУ`, `ССЫЛКА`, `ТИП`/`ТИПЗНАЧЕНИЯ`, `НЕОПРЕДЕЛЕНО`,
группирующие наборы, строковые и математические функции, `СРЕДНЕЕ`, функции
извлечения частей даты, виртуальные таблицы регистров бухгалтерии и расчёта,
...
группировкам. `ИЕРАРХИЯ` пока отклоняется.
