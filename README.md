...
- **Разворачивание ограничений** (`open_sdbl::access`).
  ...
  пишут шаблоны «1С:Документооборота», — и тогда псевдоним берётся оттуда. `read_access` объединяет роли пользователя по
...
списка с ограничением; `ТекущийВнешнийПользователь` получает пустую ссылку.
