...
Компилируются только читающие запросы. Поддержаны `ВЫБРАТЬ` с соединениями,
объединениями, группировкой и агрегатами, вложенные запросы, пакеты с
временными таблицами, условные выражения, параметры запроса и сеанса,
разыменование ссылок любой глубины (в том числе через составные,
где путь продолжается внутри каждой цели), представления значений, отбор по
типу ссылки `ССЫЛКА`, функции типов `ТИП` и `ТИПЗНАЧЕНИЯ`, литерал
`НЕОПРЕДЕЛЕНО`, несколько источников через запятую, итоги `ИТОГИ … ПО`
(в том числе `ОБЩИЕ`, `ИЕРАРХИЯ` и `ТОЛЬКО ИЕРАРХИЯ`), виртуальные таблицы
регистров сведений и накопления (срез первых и последних, остатки, обороты,
остатки и обороты), таблицы журналов документов и критериев отбора,
табличные части — и отдельным источником, и проекцией, которая отвечает
вложенным результатом, — а также построчные ограничения доступа
`РАЗРЕШЕННЫЕ`.

Не поддержаны, в частности: `АВТОУПОРЯДОЧИВАНИЕ`, `ИЕРАРХИЯ` в
`УПОРЯДОЧИТЬ ПО`, группирующие наборы, `СТРОКА(x)` (платформа возвращает
локализованное представление), виртуальные таблицы регистров бухгалтерии и
расчёта и внешние источники данных. `Остатки` доступны только у регистра с
остатками: у оборотного регистра это диагностика. Ограничения доступа не
читаются из ролей: условия задаёт приложение через callback. Проект
развивается и пока не является полной заменой языка запросов платформы 1С.

Построчная сверка с Синтакс-помощником платформы 8.3.27 вынесена в отдельный
документ: **[Язык запросов 1С: статус поддержки в
open-sdbl](docs/query-language-support.md)**. Для каждой конструкции там указан
статус (✅ поддерживается, 🟡 частично, ❌ нет), диагностика компилятора и
страница справки, по которой конструкция проверена. Ниже — те же возможности с
примерами.

Имена результирующих колонок можно задавать через `КАК`/`AS`. Источник без
псевдонима адресуется полным именем — `ВЫБРАТЬ Справочник.Товары.Наименование
ИЗ Справочник.Товары`; с псевдонимом полное имя не разрешается, потому что
псевдоним заменяет имя, — так же отвечает платформа. Табличные части
документов и справочников доступны по полному имени
`Тип.Объект.ТабличнаяЧасть`; стандартные поля строки называются `Ссылка` и
`НомерСтроки`. Табличную часть можно и спроецировать — `Д.Товары`,
`Д.Товары.(Поле, …)` или `Д.Товары.*`: платформа отвечает на такую колонку
вложенным результатом, поэтому `CompiledQuery` возвращает второй оператор в
`nested` вместе со связью по ссылке владельца (см. «Вложенные результаты»).
Колонку табличной части можно назвать и в условии — `ГДЕ Д.Товары.Количество
> 2` компилируется в коррелированный `EXISTS`, и строка владельца отвечает
один раз, сколько бы её строк ни подошло. Например:
...
В `ПО`/`ON` соединения требуется хотя бы одно равенство полей разных
источников — прямых или полученных одним разыменованием. К нему через `И`/`AND`
можно добавлять скалярные фильтры: сравнения, `В`/`IN`, `ЗНАЧЕНИЕ`/`VALUE`,
проверки NULL, функции дат и `ВЫРАЗИТЬ` к скалярному типу. Эти фильтры остаются
в `ON`, поэтому семантика внешних соединений не меняется. Не допускается
`.Реквизит` после `ВЫРАЗИТЬ(… КАК Справочник.X)`.

Конструктор даты и функции периодов поддерживаются на русском и английском:

```sql
ВЫБРАТЬ
    ДАТАВРЕМЯ(2026, 9, 2, 12, 30, 0) КАК Момент,
    НАЧАЛОПЕРИОДА(Документ.Дата, МЕСЯЦ) КАК НачалоМесяца,
    КОНЕЦПЕРИОДА(Документ.Дата, МЕСЯЦ) КАК КонецМесяца,
    ДОБАВИТЬКДАТЕ(Документ.Дата, ДЕНЬ, 3) КАК ЧерезТриДня,
    РАЗНОСТЬДАТ(Документ.Дата, &Сегодня, ДЕНЬ) КАК Прошло
ИЗ
...
`НАЧАЛОПЕРИОДА`/`BEGINOFPERIOD` и `КОНЕЦПЕРИОДА`/`ENDOFPERIOD` доступны
`МИНУТА`, `ЧАС`, `ДЕНЬ`, `НЕДЕЛЯ`, `ДЕКАДА`, `МЕСЯЦ`, `КВАРТАЛ`, `ПОЛУГОДИЕ`,
`ГОД` и английские эквиваленты; конец периода — его последняя секунда.
Неделя начинается в понедельник: региональная настройка первого дня недели
пока не входит в `MetadataSnapshot`. `ДОБАВИТЬКДАТЕ`/`DATEADD` добавляет ещё
`СЕКУНДА`, принимает любое числовое выражение как количество и «прижимает»
конец месяца (31 января плюс месяц — 29 февраля). `РАЗНОСТЬДАТ`/`DATEDIFF`
принимает `СЕКУНДА`, `МИНУТА`, `ЧАС`, `ДЕНЬ`, `МЕСЯЦ`, `КВАРТАЛ`, `ГОД` и
считает пересечённые границы единицы, как платформа: между
`31.12.2020 23:59:59` и `01.01.2021` один день, один месяц и один год.
Периоды виртуальных таблиц принимают эти функции и параметры внутри них:
`Обороты(НАЧАЛОПЕРИОДА(&П, МЕСЯЦ), КОНЕЦПЕРИОДА(&П, МЕСЯЦ))`.

Части даты `ГОД`, `КВАРТАЛ`, `МЕСЯЦ`, `ДЕНЬГОДА`, `ДЕНЬ`, `НЕДЕЛЯ`,
`ДЕНЬНЕДЕЛИ`, `ЧАС`, `МИНУТА`, `СЕКУНДА` (`YEAR` … `SECOND`) возвращают
целое число и годятся в `СГРУППИРОВАТЬ ПО`: `ГОД(Документ.Дата) КАК Год`.
`ДЕНЬНЕДЕЛИ` считает понедельник первым днём, `НЕДЕЛЯ` нумерует недели как
платформа (первая неделя — та, в которую попадает 1 января, недели с
понедельника), а не по ISO 8601.
...
Источники можно перечислять через запятую (`ИЗ Справочник.Товары КАК Т,
Справочник.Клиенты КАК К ГДЕ Т.Клиент = К.Ссылка`): каждый элемент списка
может нести свои соединения, список компилируется в цепочку `CROSS JOIN` в
порядке записи, а условие соединения видит только источники своего элемента,
как в 1С.
...
`ПОЛНОЕ СОЕДИНЕНИЕ` компилируется в нативный `FULL JOIN` и ведёт себя как
остальные соединения: его можно ставить в цепочку, группировать и
агрегировать, а разыменование в его `ПО` выносится внутрь своей стороны.
Платформа вместо `FULL JOIN` шлёт разложение на `UNION ALL` внутреннего
соединения и двух антисоединений, но на равенстве обе формы отвечают
одинаково — сверено пробами построчно. Один и тот же
объект можно присоединять несколько раз под разными псевдонимами, а
разыменование работает через любой источник цепочки — в проекции и `ГДЕ`
сколько угодно шагов, в самом `ПО` один:

```sql
ВЫБРАТЬ Док.Номер, Строки.НомерСтроки, ЦФО.Наименование, Строки.ЦФО.Код
...
`ПОЛНОЕ СОЕДИНЕНИЕ` группируется и агрегируется наравне с остальными.
...
Поддерживаются агрегаты `СУММА`, `СРЕДНЕЕ`, `МИНИМУМ`, `МАКСИМУМ` и
`КОЛИЧЕСТВО([РАЗЛИЧНЫЕ] …)`; `СРЕДНЕЕ` компилируется в `AVG`, и точность
результата определяет СУБД: PostgreSQL возвращает полное `numeric`, SQL Server
даёт шесть знаков после запятой для `numeric` и целое среднее для целочисленных
литералов.

Предложение `ИТОГИ … ПО …` даёт итоговые строки в порядке обхода
`ОбходРезультатаЗапроса.Прямой`: общий итог, затем итог каждой группы перед
её строками, группы идут в порядке первого появления в упорядоченной
выборке, как на платформе:

```sql
ВЫБРАТЬ Товары.Номенклатура КАК Номенклатура, Товары.Склад КАК Склад, Товары.Количество КАК Количество
ИЗ Документ.Реализация.Товары КАК Товары
УПОРЯДОЧИТЬ ПО Количество УБЫВ
ИТОГИ СУММА(Количество), КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Склад) КАК Склад ПО ОБЩИЕ, Номенклатура;
```

В итоговой строке заполнены контрольные точки своего и верхних уровней и
колонки итоговых полей (агрегат пишется в колонку своего аргумента, выражение
— в колонку из `КАК`), остальные колонки `NULL`. Опция
`CompileOptions::totals_level(true)` добавляет колонку `__level`, равную
`Уровень()` платформы; консоль включает её. `ПЕРИОДАМИ` принимается, но строк
не добавляет: платформа дополняет пустые периоды только при обходе по
группировкам. `ПО Товар ИЕРАРХИЯ` и `ТОЛЬКО ИЕРАРХИЯ` добавляют итоги по
папкам-предкам через рекурсивную CTE по `_ParentIDRRef` с уровнями, как у
платформы; соседние папки идут в порядке первого появления строк под ними,
а платформа сортирует их по полям `УПОРЯДОЧИТЬ ПО`, вычисленным для самих
папок. Стандартные поля `ParentID` и `OwnerID` отзываются также на
`Родитель` и `Владелец`.

Оператор `ССЫЛКА`/`REFS` проверяет тип ссылки: `ГДЕ Документ.Регистратор
ССЫЛКА Документ.РеализацияТоваровУслуг`. Составное поле сравнивает `_RTRef` с
номером типа, поле с единственной целью даёт `ИСТИНА` и для пустой ссылки, а
тип, которого поле не может содержать, отклоняется диагностикой, как в 1С.

Виртуальная таблица `ОстаткиИОбороты` выдаёт на каждый ресурс начальный
остаток, приход, расход, оборот и конечный остаток за интервал. Периодичность
у неё пока не поддержана.

Виртуальные таблицы регистра накопления `Остатки` и `Обороты` суммируют
ресурсы по тем измерениям, которые запрос не читает, как это делает платформа,
а `Обороты` принимают периодичность от `Секунда` до `Год` и выдают поле
`Период`.

Стандартные поля `ЭтоГруппа` и `Предопределенный` вычисляются так же, как их
считает платформа: первое — отрицание хранимой колонки `Folder`, второе —
непустой `PredefinedID`. Поле `ИмяПредопределенныхДанных` переводит хранимый
идентификатор в имя предопределённого элемента, объявленное в конфигурации:
непредопределённая запись отвечает пустой строкой, а непопавшая строка
соединения — `NULL`.

Ветки `ВЫБОР` и аргументы `ЕСТЬNULL` могут быть разных типов: результат —
значение составного типа, и в проекции оно раскладывается по членам, как это
делает платформа.

Подзапрос в условии видит строку внешнего оператора: `ГДЕ ЛОЖЬ В (ВЫБРАТЬ
ПЕРВЫЕ 1 ЛОЖЬ ИЗ РегистрСведений.Х КАК Р ГДЕ Р.Объект = Файлы.Ссылка)`
компилируется в коррелированный подзапрос.

Критерий отбора читается как источник: `ИЗ КритерийОтбора.Х(&Значение) КАК К`
объединяет по одному выбору на каждое поле состава критерия, а поле `Ссылка`
несёт найденный объект.

Журналы документов читаются как источник: `ИЗ ЖурналДокументов.Х КАК Ж` даёт
поля `Ссылка`, `Тип`, `Дата`, `Номер`, `ПометкаУдаления`, `Проведен` и графы
журнала. `Ссылка` — ссылка зарегистрированного документа, поэтому её можно
разыменовывать и соединять.

`ВЫБОР` принимает простую форму `ВЫБОР <выражение> КОГДА <значение> ТОГДА …`:
каждая альтернатива сравнивается с выражением по обычным правилам сравнения.

Источник соединения может нести собственные соединения до своего `ПО`, как
это позволяет 1С: `A ЛЕВОЕ СОЕДИНЕНИЕ B ЛЕВОЕ СОЕДИНЕНИЕ C ПО … ПО …`.
Группа компилируется в равносильную плоскую цепочку; сочетания, где это
изменило бы результат, отклоняются с диагностикой.

Приоритет `НЕ` снят с платформы: он слабее любого сравнения и сильнее `И`,
поэтому `ГДЕ НЕ Т.Цена = 10` отрицает сравнение целиком.

Поле составного типа сравнивается со значением так же, как это делает
платформа: предикат собирается по физическим членам — `_TYPE` против тега
типа значения и член этого типа против самого значения. Работают `=`, `<>`
и `В (…)`, в том числе со списком значений разных типов.

Строковые и математические функции платформы поддержаны: `ПОДСТРОКА`,
`ДлинаСтроки`, `СокрЛП`/`СокрЛ`/`СокрП`, `ВРег`/`НРег`, `Лев`/`Прав`,
`СтрНайти`, `СтрЗаменить`, `Окр`, `Цел`, а также `Sqrt`, `Exp`, `Log`,
`Log10`, `Pow` и тригонометрические. Семантика снята с платформы: позиции
считаются с единицы, `СтрНайти` возвращает `0`, когда подстроки нет,
`ДлинаСтроки` учитывает концевые пробелы, `Окр` округляет от нуля, `Цел`
отсекает к нулю, `Log` — натуральный логарифм.

`В ИЕРАРХИИ` отбирает всё поддерево: `ГДЕ Т.Ссылка В ИЕРАРХИИ (ВЫБРАТЬ
Г.Ссылка ИЗ Справочник.Товары КАК Г ГДЕ Г.Наименование = "Мебель")` вернёт
саму папку, её подпапки и их элементы. Спуск выполняет рекурсивная CTE по
колонке родителя, проверка идёт через `EXISTS`. Пустая ссылка охватывает весь
справочник, а справочник без иерархии вырождается в обычное вхождение — так же
отвечает платформа. Слева допускается и составная ссылка: сравнивается её
идентификатор, а тип обязан совпасть с типом семян, потому что значение
другого типа ни под каким семенем не лежит.

`УПОРЯДОЧИТЬ ПО` принимает не только поля и псевдонимы, но и выражения:
`УПОРЯДОЧИТЬ ПО ВЫБОР КОГДА Цена > 10 ТОГДА 0 ИНАЧЕ 1 КОНЕЦ, Цена УБЫВ`. В
ветви с соединением, группировкой или объединением порядок задаётся по
колонкам результата, поэтому там ключ обязан быть полем проекции или
псевдонимом.

`ТИПЗНАЧЕНИЯ`/`VALUETYPE` возвращает тип значения, `ТИП`/`TYPE` — тип по
имени: `ГДЕ ТИПЗНАЧЕНИЯ(Т.Объект) = ТИП(Справочник.Товары)`. Аргумент `ТИП` —
`Строка`, `Число`, `Дата`, `Булево` или объект метаданных. Колонка результата
имеет вид `Type` и хранит пять байт: тег типа платформы и номер таблицы
ссылки; консоль печатает имя (`Справочник.Товары`, `Строка`, `Неопределено`),
а приложение разбирает значение через `TypeValue::decode`. Составное поле
читает члены `_TYPE` и `_RTRef`, поле с одним типом даёт константу, `NULL`
даёт тип `Null`, поэтому результат никогда не пуст.

Литерал `НЕОПРЕДЕЛЕНО`/`UNDEFINED` компилируется в `NULL`. Сравнение с ним
сравнивает тип значения с типом «Неопределено»: `ГДЕ Т.Объект =
НЕОПРЕДЕЛЕНО` отбирает строки с незаполненным составным полем, а сравнение
поля, которое не может хранить это значение, даёт константный предикат, как и
платформа, возвращающая пустую выборку. В результате `Неопределено`
неотличимо от `NULL` — это отличие от 1С.
...
которые читает само условие, не фильтруются. Сырой SQL в ограничении
невозможен.
