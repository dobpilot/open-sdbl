...
памяти, ограниченный `Limits::config_resource_limit` и
`Limits::config_retained_byte_limit`: при превышении возвращается типизированная
ошибка, а не OOM. Тем, кому нужны только имена, по-прежнему подходит
`metadata()`.
...
Граница доверия сохраняется: условие ограничения — вход приложения, а не
пользователя. Таблицы, которые читает само условие, не фильтруются
рекурсивно и в запрос не попадают, а параметры запроса условию не видны;
текст пользователя в этот путь не попадает. За условия отвечает
приложение.
