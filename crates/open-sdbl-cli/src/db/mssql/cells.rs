//! Decoding TDS values into the cells the CLI prints.
...
pub(super) fn should_disconnect_after_mssql_error(error: &CliError) -> bool {
    error.requires_mssql_disconnect()
}

pub(super) fn mssql_row(row: &tiberius::Row, column_count: usize) -> Result<Vec<Cell>, CliError> {
...
pub(super) fn decode_mssql_cell(data: &ColumnData<'_>) -> Cell {
