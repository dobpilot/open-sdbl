...
use crate::limits::CONFIG_DECODE_BATCH_SIZE;
use crate::pipeline::{
...
impl<'transaction> PostgresMetadataSource<'transaction> {
    pub(super) fn new(transaction: Transaction<'transaction>) -> Self {
        Self {
    ...
    pub(super) fn transaction(&self) -> Result<&Transaction<'transaction>, CliError> {
...
pub(super) async fn verify_transaction(transaction: &Transaction<'_>) -> Result<(), CliError> {
...
pub(super) fn decode_catalog_rows(rows: Vec<Row>) -> Result<Vec<LiveTable>, CliError> {
    let values = rows
        .into_iter()
        .map(|row| {
            Ok([
