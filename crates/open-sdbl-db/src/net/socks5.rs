//! The SOCKS5 transport a database connection may be routed through.
...
use crate::credentials::EnvironmentSecret;
use crate::error::DbError;

/// A SOCKS5 proxy to route a database connection through.
...
pub struct Socks5Proxy {
    ...
    pub port: u16,
    /// The user to authenticate to the proxy as, when it demands one.
    pub username: Option<String>,
}

/// Reads a `HOST:PORT` or `[IPv6]:PORT` proxy address.
///
/// The error is a short reason, meant to be shown to the operator next to
/// the text that was rejected.
pub fn parse_socks5_proxy(value: &str) -> Result<Socks5Proxy, &'static str> {
...
/// The proxy password to use, validated for the SOCKS5 wire format.
///
/// Answers `None` when the proxy needs no authentication.
pub fn socks5_password(
...
/// Opens a TCP stream to `target_host:target_port` through the proxy.
pub async fn connect_socks5(
    ...
    let request =
        socks5_connect_request(target_host, target_port).map_err(DbError::socks5_connection)?;
    ...
    match timeout(connection_timeout, negotiation).await {
        ...
        Ok(Err(error)) => Err(DbError::socks5_connection(error)),
        Err(_) => Err(DbError::socks5_connection(format!(
