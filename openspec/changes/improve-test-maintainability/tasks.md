...
- [x] 1.1 Inventory the remaining tests in `crates/open-sdbl-cli/src/main.rs`
  and move each provider, argument, credential, network, output, progress, and
  pipeline test beside its owning module.
- [x] 1.2 Keep only orchestration tests in `main.rs` and remove coverage that
  duplicates module-local or integration tests.

## 2. Representative fuzzing

- [x] 2.1 Extend the fixed query fuzz snapshot so valid reference
  dereferences and tabular-section sources are reachable.
- [x] 2.2 Add a bounded CI build/check of `fuzz/Cargo.toml`; do not run an
