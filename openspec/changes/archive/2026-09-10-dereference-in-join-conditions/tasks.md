...
- [x] 1.1 Resolve one-hop dereferences in join anchors and additional
  predicates, keeping the anchor rule and scope checks on the base scope;
  reject dereferences in `ПОЛНОЕ СОЕДИНЕНИЕ` conditions.
- [x] 1.2 Render dereference joins as parenthesized groups next to their
  source when a condition uses one; keep the flat form otherwise.

## 2. Verification and documentation

- [x] 2.1 Goldens on both dialects: tabular section joined to its owner
...
- [x] 2.2 Update README and `docs/query-language-support.md`; run
